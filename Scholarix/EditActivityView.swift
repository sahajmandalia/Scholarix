...
struct EditActivityView: View {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            LinearGradient(colors: [Color.green.opacity(0.05), Color.orange.opacity(0.05)], startPoint: .topLeading, endPoint: .bottomTrailing).ignoresSafeArea()
            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    FormCard(title: "BASIC INFO") {
                        HStack {
                            Image(systemName: "star").foregroundColor(.orange).frame(width: 24)
                            TextField("Activity Name", text: $title)
                        }.formRow()
                        Divider()
                        HStack {
                            Image(systemName: "person.text.rectangle").foregroundColor(.gray).frame(width: 24)
                    ...
                    FormCard(title: "TIMELINE") {
                        DatePicker("Start Date", selection: $startDate, displayedComponents: .date).formRow()
                        Divider()
                        Toggle("Currently Ongoing", isOn: $isOngoing).padding(.vertical, 4).formRow()
                        if !isOngoing {
                            Divider()
                            DatePicker("End Date", selection: $endDate, displayedComponents: .date).formRow()
                        }
                    }
                    
                    FormCard(title: "IMPACT") {
                        HStack {
                            Image(systemName: "clock").foregroundColor(.gray).frame(width: 24)
                            TextField("Total Hours (Optional)", text: $hoursString).keyboardType(.decimalPad)
                        }.formRow()
                        Divider()
                        VStack(alignment: .leading, spacing: 8) {
                            Text("Description / Achievements").font(.caption).foregroundColor(.secondary).padding(.top, 4)
                            TextEditor(text: $description).frame(height: 100).padding(4).background(Color(.systemGray6)).cornerRadius(8)
                        }.padding(.bottom, 8)
                    }
